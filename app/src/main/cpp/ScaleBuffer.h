...
public:
    ~ScaleBuffer() override
    {
        delete(m_pBins);
    }

    void setOutputWidth(int outputWidth, float minFreq, float maxFreq) override
    {
        delete (m_pOutput);
    ...
    float XtoFreq(float x) const override
    {
        return scaleXtoFreq.forward(x);
    }

    float FreqToX(float freq) const override
    {
